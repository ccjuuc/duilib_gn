...
namespace DuiLib {
	...
	bool CListUI::SetItemIndex(CControlUI* pControl, int iIndex)
	{
		...
		int iMinIndex = std::min(iOrginIndex, iIndex);
		int iMaxIndex = std::max(iOrginIndex, iIndex);
	...
	bool CListUI::Add(CControlUI* pControl)
	{
		...
		if (pControl->GetInterface(_T("ListHeader")) != NULL) {
			...
			m_ListInfo.nColumns = std::min(m_pHeader->GetCount(), UILIST_MAX_COLUMNS);
	...
	void CListBodyUI::SetScrollPos(SIZE szPos, bool bMsg)
	{
		...
		if (m_pOwner) {
			...
			pInfo->nColumns = std::min(pHeader->GetCount(), UILIST_MAX_COLUMNS);
	...
	void CListBodyUI::SetPos(RECT rc, bool bNeedInvalidate)
	{
		...
		for (int it1 = 0; it1 < m_items.GetSize(); it1++) {
			...
			sz.cx = std::max(sz.cx, (LONG)0);
			if (sz.cx < pControl->GetMinWidth()) sz.cx = pControl->GetMinWidth();
			if (sz.cx > pControl->GetMaxWidth()) sz.cx = pControl->GetMaxWidth();
			cxNeeded = std::max((LONG)cxNeeded, sz.cx);
			nEstimateNum++;
		}
		...
		if (m_pOwner) {
			...
			if (pHeader != NULL && pHeader->GetCount() > 0) {
				cxNeeded = std::max((LONG)0, pHeader->EstimateSize(CDuiSize(rc.right - rc.left, rc.bottom - rc.top)).cx);
				if (m_pHorizontalScrollBar && m_pHorizontalScrollBar->IsVisible())
				{
		...
		int cyNeeded = 0;
		int cyExpand = 0;
		if (nAdjustables > 0) cyExpand = std::max((LONG)0, (szAvailable.cy - cyFixed) / nAdjustables);
		...
		for (int it2 = 0; it2 < m_items.GetSize(); it2++) {
			...
			if (sz.cy == 0) {
				...
				if (iAdjustable == nAdjustables) {
					sz.cy = std::max((LONG)0, szRemaining.cy - rcPadding.bottom - cyFixedRemaining);
				}
				if (sz.cy < pControl->GetMinHeight()) sz.cy = pControl->GetMinHeight();
				if (sz.cy > pControl->GetMaxHeight()) sz.cy = pControl->GetMaxHeight();
			}
			...
			sz.cx = std::max((LONG)cxNeeded, szAvailable.cx - rcPadding.left - rcPadding.right);
	...
	SIZE CListHeaderUI::EstimateSize(SIZE szAvailable)
	{
		...
		if (cXY.cy == 0 && m_pManager != NULL) {
			for (int it = 0; it < m_items.GetSize(); it++) {
				cXY.cy = std::max(cXY.cy, static_cast<CControlUI*>(m_items[it])->EstimateSize(szAvailable).cy);
			}
			int nMin = m_pManager->GetDefaultFontInfo()->tm.tmHeight + 6;
			cXY.cy = std::max(cXY.cy, (LONG)nMin);
		}
	...
	void CListHeaderUI::SetPos(RECT rc, bool bNeedInvalidate)
	{
		...
		int cxExpand = 0;
		int cxNeeded = 0;
		if (nAdjustables > 0) cxExpand = std::max((LONG)0, (szAvailable.cx - cxFixed) / nAdjustables);
		...
		for (int it2 = 0; it2 < m_items.GetSize(); it2++) {
			...
			if (sz.cx == 0) {
				...
				// Distribute remaining to last element (usually round-off left-overs)
				if (iAdjustable == nAdjustables) {
					sz.cx = std::max((LONG)0, szRemaining.cx - rcPadding.right - cxFixedRemaining);
				}
				if (sz.cx < pControl->GetMinWidth()) sz.cx = pControl->GetMinWidth();
				if (sz.cx > pControl->GetMaxWidth()) sz.cx = pControl->GetMaxWidth();
			}
	...
	void CListContainerElementUI::DrawItemBk(HDC hDC, const RECT& rcItem)
	{
		...
		if (IsSelected()) {
			if (!pInfo->sSelectedImage.IsEmpty()) {
				bool bDrawOk = DrawImage(hDC, (LPCTSTR)pInfo->sSelectedImage);
				if(!pInfo->sSelectedForeImage.IsEmpty()) {
					DrawImage(hDC, (LPCTSTR)pInfo->sSelectedForeImage);
				}
				if(bDrawOk) return;
			}
		}
		if ((m_uButtonState & UISTATE_HOT) != 0) {
			if (!pInfo->sHotImage.IsEmpty()) {
				bool bDrawOk = DrawImage(hDC, (LPCTSTR)pInfo->sHotImage);
				if(!pInfo->sHotForeImage.IsEmpty()) {
					DrawImage(hDC, (LPCTSTR)pInfo->sHotForeImage);
				}
				if(bDrawOk) return;
			}
		}
		...
		if (!m_sBkImage.IsEmpty()) {
			if (!pInfo->bAlternateBk || m_iIndex % 2 == 0) {
				...
				if(!pInfo->sForeImage.IsEmpty()) {
					DrawImage(hDC, (LPCTSTR)pInfo->sForeImage);
				}
			}
		}
		...
		if (m_sBkImage.IsEmpty()) {
			if (!pInfo->sBkImage.IsEmpty()) {
				bool bDrawOk = DrawImage(hDC, (LPCTSTR)pInfo->sBkImage);
				if(!pInfo->sForeImage.IsEmpty()) {
					DrawImage(hDC, (LPCTSTR)pInfo->sForeImage);
				}
			}
