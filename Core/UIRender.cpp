...
namespace DuiLib {
	...
	void GdiplusDrawRoundRect(HDC hDC, float x, float y, float width, float height, float arcSize, float lineWidth, Gdiplus::Color lineColor, bool fillPath, Gdiplus::Color fillColor, int nStyle)
	{
		...
		// �����ͼ·��
		roundRectPath.AddLine(x + arcSize, y, x + width - arcSize, y);  // ��������
		roundRectPath.AddArc(x + width - arcDiameter, y, arcDiameter, arcDiameter, 270, 90); // ����Բ��

		roundRectPath.AddLine(x + width, y + arcSize, x + width, y + height - arcSize);  // �Ҳ�����
		roundRectPath.AddArc(x + width - arcDiameter, y + height - arcDiameter, arcDiameter, arcDiameter, 0, 90); // ����Բ��

		roundRectPath.AddLine(x + width - arcSize, y + height, x + arcSize, y + height);  // �ײ�����
		roundRectPath.AddArc(x, y + height - arcDiameter, arcDiameter, arcDiameter, 90, 90); // ����Բ��

		roundRectPath.AddLine(x, y + height - arcSize, x, y + arcSize);  // �������
		roundRectPath.AddArc(x, y, arcDiameter, arcDiameter, 180, 90); // ����Բ��
	...
	void CRenderEngine::DrawHtmlText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, RECT* prcLinks, CDuiString* sLinks, int& nLinkRects, int iFont, UINT uStyle)
	{
					...
					case _T('a'):  // Link
						{
							...
							if( pFontInfo->bUnderline == false ) {
								...
								cyLine = std::max((LONG)cyLine, pTm->tmHeight + pTm->tmExternalLeading + (int)aPIndentArray.GetAt(aPIndentArray.GetSize() - 1));
							}
							ptLinkStart = pt;
							bInLink = true;
						}
							...
							if( sName.IsEmpty() ) { // Italic
								...
								if( pImageInfo ) {
									...
									if( pt.x + iWidth > rc.right && pt.x > rc.left && (uStyle & DT_SINGLELINE) == 0 ) {
										...
										cyLine = std::max(iHeight, cyLine);
										pt.x += iWidth;
										cyMinHeight = pt.y + iHeight;
										cxMaxWidth = std::max((LONG)cxMaxWidth, pt.x);
									}
								}
					...
					case _T('p'):  // Paragraph
						{
							...
							cyLine = std::max((LONG)cyLine, pTm->tmHeight + pTm->tmExternalLeading + cyLineExtra);
						}
						break;
					case _T('r'):  // Raw Text
						{
				...
				case _T('a'):
					{
						if( iLinkIndex < nLinkRects ) {
							if( !bLineDraw ) ::SetRect(&prcLinks[iLinkIndex], ptLinkStart.x, ptLinkStart.y, std::min(pt.x, rc.right), pt.y + pTm->tmHeight + pTm->tmExternalLeading);
							iLinkIndex++;
						}
						aColorArray.Remove(aColorArray.GetSize() - 1);
						DWORD clrColor = dwTextColor;
		...
		if( (uStyle & DT_CALCRECT) != 0 ) {
			rc.bottom = std::max((LONG)cyMinHeight, pt.y + cyLine);
			rc.right = std::min(rc.right, (LONG)cxMaxWidth);
		}
