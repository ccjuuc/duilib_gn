...
namespace DuiLib
{
	...
	void CVerticalLayoutUI::SetPos(RECT rc, bool bNeedInvalidate)
	{
		...
		for( int it1 = 0; it1 < m_items.GetSize(); it1++ ) {
			...
			cxNeeded = std::max((LONG)cxNeeded, sz.cx + rcPadding.left + rcPadding.right);
			nEstimateNum++;
		}
	...
	RECT CVerticalLayoutUI::GetThumbRect(bool bUseNew) const
	{
		if( (m_uButtonState & UISTATE_CAPTURED) != 0 && bUseNew) {
			if( m_iSepHeight >= 0 ) 
				return CDuiRect(m_rcNewPos.left, std::max(m_rcNewPos.bottom - m_iSepHeight, m_rcNewPos.top), 
				m_rcNewPos.right, m_rcNewPos.bottom);
			else 
				return CDuiRect(m_rcNewPos.left, m_rcNewPos.top, m_rcNewPos.right, 
				std::min(m_rcNewPos.top - m_iSepHeight, m_rcNewPos.bottom));
		}
		else {
			if( m_iSepHeight >= 0 ) 
				return CDuiRect(m_rcItem.left, std::max(m_rcItem.bottom - m_iSepHeight, m_rcItem.top), m_rcItem.right, 
				m_rcItem.bottom);
			else 
				return CDuiRect(m_rcItem.left, m_rcItem.top, m_rcItem.right, 
				std::min(m_rcItem.top - m_iSepHeight, m_rcItem.bottom));
